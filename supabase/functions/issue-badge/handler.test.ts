...
Deno.test("Deno signer reproduces the frozen test vector (parity with the Node signer)", async () => {
  ...
  assertEquals(
    ed25519PublicKeyMultibase(ed.getPublicKey(TEST_SEED)),
    EXPECTED_PUBLIC_KEY_MULTIBASE,
  );
...
Deno.test("Deno verifier accepts a fresh signature and rejects tampering", async () => {
  ...
  const tampered = JSON.parse(JSON.stringify(signed)) as Record<
    ...
    unknown
  >;
  (tampered.credentialSubject as { achievement: { name: string } }).achievement
    .name = "Forged";
...
Deno.test("buildAndSignCredential self-verifies and embeds a revocation status entry", async () => {
  ...
  const { credential } = await buildAndSignCredential({
    spec: {
      ...
      evidenceUrls: ["https://redex.education/evidence/so-1/ev-1"],
    },
    issuer: { did: TEST_ISSUER_DID, keyId: TEST_KEY_ID, secretKeyMultibase },
    credentialId: "https://redex.education/credentials/abc",
    statusListIndex: 42,
    created: TEST_CREATED,
    endpoints: resolveIssuerEndpoints(TEST_ISSUER_DID),
  });
  const status = (credential as {
    credentialStatus: { statusListIndex: string; type: string };
  })
    .credentialStatus;
  assertEquals(status.type, "BitstringStatusListEntry");
  assertEquals(status.statusListIndex, "42");
  assertEquals(
    (credential as { evidence: { id: string }[] }).evidence[0]!.id,
    "https://redex.education/evidence/so-1/ev-1",
  );
...
Deno.test('Deno builder defaults evidence[].type to ["Evidence"] (parity with the Node builder)', () => {
  const c = buildUnsignedCredential({
    credentialId: "https://redex.education/credentials/z",
    issuer: { id: TEST_ISSUER_DID },
    achievement: { id: "https://redex.education/achievements/z", name: "Z" },
    validFrom: TEST_CREATED,
    evidence: [{ id: "https://redex.education/evidence/2" }], // no type
  });
...
Deno.test("readClaims parses roles/org_id from a JWT payload", () => {
  ...
  const b64 = btoa(JSON.stringify(payload)).replace(/\+/g, "-").replace(
    /\//g,
    "_",
  ).replace(/=+$/, "");
...
Deno.test("bodySchema requires a valid action and rejects malformed payloads", () => {
  assertFalse(
    bodySchema.safeParse({ recipient_user_id: "not-a-uuid" }).success,
  ); // no action
  assertFalse(
    bodySchema.safeParse({ action: "rollup", candidate_user_id: "x" }).success,
  ); // bad uuid
  assert(
