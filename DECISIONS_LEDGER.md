...
- OB 3.0 / W3C Verifiable Credentials: pin a **proof suite** (recommend `eddsa-rdfc-2022` / Ed25519), **issuer `did:web:academy.goredex.com`** (or a stable hosted issuer profile served by a Worker), and a **hosted status list** for revocation (BitstringStatusList / StatusList2021). Define a **key-rotation** plan (issuer profile lists current + retired keys).
