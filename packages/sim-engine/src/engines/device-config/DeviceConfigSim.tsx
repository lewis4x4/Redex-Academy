...
export function DeviceConfigSim({
  ...
  return (
    <SimFrame mode={mode} title={title}>
      <nav aria-label="Screens" className="flex flex-wrap gap-2">
        {spec.screens.map((s) => (
          <button
            ...
            className="rounded border border-slate-300 px-2 py-1 text-sm aria-[current=page]:bg-slate-900 aria-[current=page]:text-white"
          >
            {t(s.title_i18n)}
          </button>
        ))}
      ...
      {imageKey ? (
        <p data-screen-image={imageKey} className="font-mono text-xs text-slate-500">
